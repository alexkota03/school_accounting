...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'get_student_fees') {
    ...
    // جلب رسوم الطالب الأساسية + رسوم التوصيل مع أحدث البيانات من الجداول الأصلية
    $query = "SELECT 
                sf.id,
                CASE 
...
// دالة حساب رصيد الطالب (محدثة بالكامل)
function getStudentBalance($student_id, $db) {
    // حساب إجمالي الرسوم الأساسية بشكل ديناميكي من الجداول الأصلية
    ...
    // حساب المبلغ المتبقي (الرسوم الأساسية فقط)
...
// دالة سداد الرسوم المستحقة من المبلغ الإضافي (مصححة)
function payFromAdditionalBalance() {
    ...
    function displayFeesTable(fees) {
        ...
        if (!fees || fees.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="padding: 20px; text-align: center; color: var(--gray);">لا توجد رسوم معلقة لهذا الطالب</td></tr>';
        ...
        fees.forEach(fee => {
            ...
            html += `
                <tr>
                    <td>${fee.fee_name}</td>
                    <td>${formatCurrency(fee.final_amount)} <?php echo CURRENCY; ?></td>
                    <td>${formatCurrency(fee.paid_amount)} <?php echo CURRENCY; ?></td>
                    <td>
